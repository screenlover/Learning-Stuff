...
from button import Button, EasyButton, MediumButton, HardButton
...
class AlienInvasion:
    ...
    def __init__(self):
        ...
        self.easy_difficulty = EasyButton(self, "Easy")
        self.medium_difficulty = MediumButton(self, "Medium")
        self.hard_difficulty = HardButton(self, "Hard")
    ...
    def _check_aliens_bottom(self):
        for alien in self.aliens.sprites():
            if alien.rect.bottom >= self.settings.screen_height: # See how useful it is to separate values in specific variables, functions and classes? You have already used "screen_height" and "screen_width" multiple times, and this a value you want to constantly change to test in different screen sizes.
                self._ship_hit()
                break
    ...
    def _check_events(self): # helper methods are not meant to be used outside the class and always start with an underscore
        for event in pygame.event.get():
            if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_q): # simpler and cleaner than the original solution (i guess...)
                sys.exit()
            elif event.type == pygame.KEYDOWN and self.chosen:
                self._check_key_event(event, True)
            elif event.type == pygame.KEYUP and self.chosen:
    ...
    def _check_difficulty_button(self, mouse_pos):
        easy_clicked = self.easy_difficulty.rect.collidepoint(mouse_pos)
        medium_clicked = self.medium_difficulty.rect.collidepoint(mouse_pos)
        hard_clicked = self.hard_difficulty.rect.collidepoint(mouse_pos)
        ...
        if hard_clicked:
            ...
            self.settings.speedup_scale = 1.8
    ...
    def _create_fleet(self):
        ...
        while current_y < self.settings.screen_height - 200:
    ...
    def _bullet_sound(self):
        firing_sound = pygame.mixer.Sound("laser.mp3")
        firing_sound.play()
